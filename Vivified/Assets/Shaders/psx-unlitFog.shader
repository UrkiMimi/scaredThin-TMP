...
Shader "psx/unlit fog" {
	Properties{
		...
		_FogStartOffset("Fog Offset", Float) = 0
...
#include "UnityCG.cginc"
	...
	v2f vert(appdata_full v)
	{
		...
		o.eyePos = -UnityObjectToViewPos(v.vertex.xyz).z;
		return o;
	}
	...
	{
		float dumbFog = clamp( ( ( ( IN.eyePos ) - ( _FogStartOffset ) ) / _FogScale ) , 0.0 , 1.0 );
		float4 psxTex = tex2D(_MainTex, IN.uv_MainTex / IN.normal.r);
		float4 color = _Color * psxTex;
		color = (color * (1 - dumbFog)) + (dumbFog * _FogColor);
